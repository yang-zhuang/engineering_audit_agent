...
def extract_contract_date_from_documents(documents: List[Dict]) -> Dict[str, Dict]:
    """
    从采购合同中提取签订日期

    Args:
        documents: 采购合同文档列表（metadata列表）

    Returns:
        {original_file_path: {"signing_dates": [...]}}
    """
    prompt_name = "extract_purchase_contract_date.txt"
    results = {}

    print(f"\n--- 提取采购合同签订日期 ({len(documents)} 个文件) ---")
...
def extract_contract_items_from_documents(documents: List[Dict]) -> Dict[str, Dict]:
    """
    从采购合同中提取材料明细
    ...
    """
    prompt_name = "extract_purchase_contract_items.txt"
    results = {}

    print(f"\n--- 提取采购合同材料明细 ({len(documents)} 个文件) ---")
