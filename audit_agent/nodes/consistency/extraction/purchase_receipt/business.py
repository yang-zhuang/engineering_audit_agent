...
def extract_purchase_receipt_data_from_documents(documents: List[Dict]) -> Dict[str, Dict]:
    """
    从采购入库单中同时提取日期和材料明细

    Args:
        documents: 采购入库单文档列表（metadata列表）

    Returns:
        {original_file_path: {"单据基本信息": {...}, "供应商与入库信息": {...}, "明细数据": [...], ...}}
    """
    prompt_name = "extract_purchase_receipt_date_and_items.txt"
    results = {}

    print(f"\n--- 提取采购入库单日期和材料明细 ({len(documents)} 个文件) ---")
